    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h3 className="text-lg font-bold text-gray-900">{initial?.id ? t('businesses.edit') : t('businesses.new')}</h3>
        </div>
        <form onSubmit={submit} className="space-y-4">
          {[{l:`${t('businesses.name')} *`,k:'ad'},{l:t('businesses.phone'),k:'telefon'},{l:t('businesses.address'),k:'adres'}].map(f=>(
            <div key={f.k}>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">{t('businesses.code')} *</label>
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4">
          ...
          <h3 className="text-center text-xl font-black text-gray-900 mb-2">
            {pasif ? t('businesses.deactivateTitle') : t('businesses.activateTitle')}
          </h3>
          ...
          {pasif ? (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6">
              <p className="text-sm font-semibold text-amber-800 mb-1">{t('warning.attention')}</p>
            ...
            <div className="bg-green-50 border border-green-200 rounded-2xl p-4 mb-6">
              <p className="text-sm font-semibold text-green-800 mb-1">{t('warning.confirm')}</p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              style={{ background: pasif ? GRAD.red : GRAD.green }}
            >
              {pasif ? t('action.yesDeactivate') : t('action.yesActivate')}
            </button>
          </div>
        </div>
...
export default function IsletmelerPage() {
  ...
  const load = useCallback(async (sp = sayfa) => {
    ...
    try {
      ...
      if (filtre === 'active') p.set('aktif', 'true');
      if (filtre === 'passive') p.set('aktif', 'false');
  ...
  const handleOnayConfirm = async () => {
    ...
    setOnay(null);
    try {
      if (tip === 'pasif') { await api.delete(`/isletmeler/${id}`); toast.success(t('toast.deactivated')); }
      else                 { await api.put(`/isletmeler/${id}`, { aktif: true }); toast.success(t('toast.activated')); }
      load();
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title={t('businesses.title')} stats={[
        {icon:Building2,label:t('stat.total'),value:toplam,color:GRAD.indigo},
        {icon:Power,label:t('stat.active'),value:filtre==='active'?toplam:undefined,color:GRAD.green},
        {icon:Users,label:t('stat.passive'),value:filtre==='passive'?toplam:undefined,color:GRAD.red},
      ]} />
      ...
      <div className="p-4 sm:p-6 lg:p-8 flex-1 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input type="text" placeholder={t('businesses.search')} value={search} onChange={e=>setSearch(e.target.value)}
          ...
          <div className="flex gap-2">
            <div className="flex bg-white rounded-xl border border-gray-200 p-1">
              {[{key:'all',label:t('filter.all')},{key:'active',label:t('filter.active')},{key:'passive',label:t('filter.passive')}].map(f=>(
            ...
            <button onClick={()=>{setEditing(null);setModalOpen(true);}}
              ...
              <Plus className="w-4 h-4"/><span className="hidden sm:inline">{t('businesses.new')}</span><span className="sm:hidden">+</span>
            </button>
          </div>
        </div>
        ...
        {loading?(
          ...
          <div className="bg-white rounded-2xl p-16 text-center border border-gray-100">
            ...
            <p className="text-gray-400 font-medium">{t('businesses.notFound')}</p>
          </div>
        ):(
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
            {isletmeler.map(i=>(
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[11px] font-bold px-1.5 py-0.5 rounded-full"
                        ...
                        {i.aktif ? t('status.active') : t('status.passive')}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    {i.aktif ? (
                      ...
                      <button onClick={()=>handleAktifle(i)}
                        ...
                        style={{background:'#DCFCE7'}} title={t('action.activate')}>
